...
или если подпрограмма принимает переменное число параметров (как `printf` в Си),
для передачи части параметров используется стек.

Например, чтобы вызвать функцию `printf` потребуется подготовить параметры следующим образом:
* адрес форматной строки должен быть помещен в `r0`;
* дополнительные параметры `printf` должны быть помещены в стек, при этом нужно учитывать следующее:
** даже под параметры, передаваемые на регистрах, выделяется место на стеке, это место не используется;
...
После завершения функции `printf` стек должен быть очищен самой подпрограммой.
...
main:   stmfd   sp!, { r4, lr }
        ...
        stmfd   sp!, { r0, r1 }      // сохраняем в стек и r0, и r1
        add     r0, pc, #fmt - . - 8
        bl      printf
...
```
        stmfd   sp!, { r0, r1 }
```
сохраняет в стек регистры `r0` и `r1` причем всегда в порядке убывания номеров регистров, то есть значение регистра `r1` окажется
в стеке выше (то есть по большим адресам), чем значение регистра `r1`. Поэтому значение регистра `r0`
попадет на место в стеке, отведенное под первый параметр, и функцией `printf`
...
main:   stmfd   sp!, { r4, lr }
        ...
        stmfd   sp!, { r0, r1 }       // заносим в стек r0 и r1, scanf будет использовать значение в стеке из r1
        add     r0, pc, #sfmt - . - 8 // грузим в r0 адрес форматной строки
        bl      scanf
        add     sp, sp, #8            // чистим стек
...
main:   stmfd   sp!, { r4, lr }
        ...
        add     r0, pc, #sfmt - . - 8
        bl      scanf
        add     sp, sp, #8            // чистим стек после scanf
